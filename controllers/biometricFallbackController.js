...
export const addSecurityQuestion = async (userId, question, answer) => {
  // Check if keys exist
  let keyResult = await query(`SELECT public_key FROM vottery_biometric_keys WHERE user_id=$1`, [userId]);

  if (keyResult.rowCount === 0) {
    console.log('Keys not found, auto-registering...');
    await registerKeys(userId);
    keyResult = await query(`SELECT public_key FROM vottery_biometric_keys WHERE user_id=$1`, [userId]);
  }
